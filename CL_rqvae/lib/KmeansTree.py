...
def initialize(X, num_clusters):
    ...
    # print(num_clusters)
    # print(float(X[indices].shape)/float(num_clusters))
...
def kmeans_equal(
    ...
    final_choice=torch.full((X.shape[0],),-1,dtype=torch.int64,device=device)#X.device)
    left_index=torch.full((X.shape[0],),True,dtype=torch.bool,device=device)#X.device)
    all_ins_ids=torch.arange(X.shape[0],device=device)#X.device)
    while True:
        ...
        # print("iteration:{}, center_shift: {}.".format(iteration, center_shift))
...
class ConstructKmeansTree:
    def __init__(self,parall, device):
        self.timeout=5
        self.parall=parall
        self.device = device
    def train(self,data=None,k=4,max_iters=100,feature_ratio=0.8):
        ...
        while queue.qsize()>0:
            ...
            if len(index)<=1024:
                ...
                tstart = time.time()
                result_matrix= self._kmeans(index, self.device)
    ...
    def _minbatch(self, pcode, index):
        ...
        while dq:
            ...
            if len(index) == self.k:
                ...
                result_matrix = self._kmeans(index, self.device)# divide the index into two nodes
    ...
    def _kmeans(self,index, device):
        #index is tensor
        #make sure that index.length is greater or equal to k,data is tensor matrix
        #data is tensor
        assert len(index)%self.k==0
        cluster_size=len(index)//self.k # 262144 64 4096
        selected_feature=torch.randperm(self.data.shape[1])[:math.ceil(self.feature_ratio*self.data.shape[1])]
        choices, _ = kmeans_equal(self.data[:,selected_feature][index], cluster_size=cluster_size,num_clusters=self.k,max_iters=self.max_iters, device = device)
        result_index=torch.full((self.k,cluster_size),-1,dtype=torch.int64,device=device) #index.device)
        for c in range(self.k):
