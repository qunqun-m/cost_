    ...
    def __init__(self,
        ...
        if tree_has_generated:
            ...
            self.tree = Tree(data=data,max_iters=max_iters,feature_ratio=feature_ratio,\
                                item_num=item_num,k=k,init_way=init_way,parall=parall, codes = codes, device = self.device)
            ...
            for item_id,paths in self.tree.item_to_code.items():
                # if paths == len(self.tree.item_to_code.items()):
                #     break
                # print(item_id, paths, paths[0])
                assert len(paths)>0
        ...
        self.tree.card = self.tree.card.to(self.device)
        self.tree.code_to_item = self.tree.code_to_item.to(self.device)
        self.tree.code_to_item = self.tree.code_to_item.to(self.device)
        # for key in self.tree.code_to_item:
        #     self.tree.code_to_item[key] = self.tree.code_to_item[key].to(self.device)
